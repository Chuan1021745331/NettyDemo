...
public class StringGZIP {
	private final static Logger logger= LoggerFactory.getLogger(StringGZIP.class);
...
//		logger.info("原始的字符串为--->" + str);
//		logger.info("原始的字符串长度为------->" + b);
//
//		String ys = compress(str);
//		int a = ys.getBytes().length;
//		logger.info("压缩后的字符串长度为----->" + a);
//
//		String jy = unCompress(ys);
//		logger.info("解压后字符串为--->" + jy);
//		logger.info("解压后字符串长度为--->" + jy.getBytes().length);
//
//		BigDecimal c = new BigDecimal(b + "").divide(new BigDecimal(a + ""), 5, BigDecimal.ROUND_HALF_UP);
//		logger.info("压缩比例为" + c.multiply(new BigDecimal(100)) + "%");
//
//		// 判断
//		if (str.equals(jy)) {
//			logger.info("先压缩再解压以后字符串和原来的是一模一样的");
//		}
	}
