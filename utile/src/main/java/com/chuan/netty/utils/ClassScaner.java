...
public class ClassScaner {
	...
	public static <T> List<Class<T>> scanSubClass(Class<T> pclazz, boolean mustbeCanNewInstance) {
		...
		scanClass(classFileList,getRootClassPath());

		List<Class<T>> classList = new ArrayList<Class<T>>();
		for (File file : classFileList) {

			int start = getRootClassPath().length();
		...
		if (jarsDir.exists() && jarsDir.isDirectory()) {
			File[] jarFiles = jarsDir.listFiles(new FileFilter() {
				...
				public boolean accept(File pathname) {
					...
					return name.endsWith(".jar") && name.startsWith("hxjd");
				}
			});
	...
	public static String getRootClassPath(){
		String path="";
		try {
			path = ClassScaner.class.getClassLoader().getResource("").toURI().getPath();
			return new File(path).getAbsolutePath();
		} catch (URISyntaxException e) {
			path=ClassScaner.class.getClassLoader().getResource("").getPath();
			return new File(path).getAbsolutePath();
		}
	}
}
